...
def get_investments():
    ...
    interval_investment = int(request.args.get('investment', 1000))
    interval = request.args.get('interval', '3mo')
    stock_data = yf.download('^GSPC', start=start_month, end=datetime.date.today().isoformat(), interval=interval)

    investment_list = []

    total_net = 0
    total_invested = 0
    last_price = stock_data['Open'][0]
    locale.setlocale(locale.LC_ALL, '')
    for date, row in stock_data.iterrows():
        open_price = row['Open']
        roi = open_price / last_price
        last_price = open_price
        total_net = total_net * roi + interval_investment
        total_invested += interval_investment
